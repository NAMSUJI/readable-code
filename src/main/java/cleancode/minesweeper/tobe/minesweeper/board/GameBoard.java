...
//        openSurroundedCells(cellPosition);
    ...
    private void openAndPushCellAt(Stack<CellPosition> stack) {
        ...
        if (isLandMineCell(currendCellPosition)) {
            return;
        }

        openOneCellAt(currendCellPosition);

        if (doesCellHaveLandMineCount(currendCellPosition)) {
            return;
        }

        List<CellPosition> surroundedPositions = calculateSurroundedPositions(currendCellPosition, getRowSize(), getColSize());
